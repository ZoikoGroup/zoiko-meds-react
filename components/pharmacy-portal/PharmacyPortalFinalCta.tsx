...
/**
 * PharmacyPortalFinalCtaSection
 * Closing dark CTA banner — rounded navy card with headline, supporting
 * copy, and two CTAs (solid teal + outline).
 *
...
export default function PharmacyPortalFinalCtaSection() {
            ...
            <>
              <Reveal index={0}>
                <h2 className="font-[var(--font-plus-jakarta-sans)] text-2xl font-bold leading-tight text-white sm:text-[1.85rem]">
                  Access the portal built for{" "}
                  <span style={{ color: ACCENT }}>
                    verified pharmacy availability workflows.
                  </span>
                </h2>
              </Reveal>
              ...
              <Reveal index={1}>
                <p className="mx-auto mt-4 max-w-xl text-[14px] leading-relaxed text-[#A9B2C8]">
                  Manage your profile, signal controls, confirmation
                  requests, branches, integrations, and support through a
                  secure ZoikoMeds Pharmacy Portal.
                </p>
              </Reveal>
...
function Reveal({
  ...
  return (
    <div
      className="animate-[portalFinalCtaFadeUp_0.6s_ease-out_forwards]"
      style={{ opacity: 0, animationDelay: `${index * 100}ms` }}
    >
      {children}
      <style jsx>{`
        @keyframes portalFinalCtaFadeUp {
          from {
            opacity: 0;
