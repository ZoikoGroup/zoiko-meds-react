...
const actions = [
  ...
  {
    label: "Learn about availability confidence",
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="text-center">
          ...
          <p className="mx-auto mt-3 max-w-2xl text-[16px] leading-6 text-[#566476]">
            Stated once, plainly: what you control, and what saved searches
            never do.
          </p>
        </div>
        ...
        <div className="mt-6 flex flex-col gap-3 sm:flex-row">
          {actions.map((action) => (
            <Link
              key={action.href}
              href={action.href}
              className="inline-block rounded-xl cursor-pointer border border-[#CDD7E3] bg-white px-5 py-2.5 text-[15px] font-semibold text-[#0D1B2E] transition hover:bg-gray-50"
            >
              {action.label}
            </Link>
          ))}
        ...
        <div className="mt-6 flex items-start gap-4 rounded-xl border-l-4 border-[#0C1B30] bg-[#EEF2F7] p-4">
          ...
          <p className="text-[13px] leading-6 text-[#2B3A4F]">
            ZoikoMeds is not a pharmacy and does not prescribe, dispense, sell,
            deliver, reserve, recommend, or guarantee medicines. Saved searches
            are convenience tools — not prescriptions, refills, or clinical
            records. Availability is confidence-based and must be confirmed
