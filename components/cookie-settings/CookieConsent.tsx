...
const initialCookieCategories: CookieCategory[] = [
  {
    id: 1,
    title: "Essential",
    alwaysOn: true,
  ...
  },
  {
    ...
    title: "Functional",
    enabled: true,
    description: "Remember preferences that improve the user experience.",
    examples: "language, region, display preferences, saved UI choices.",
  },
  {
    id: 3,
    title: "Analytics",
  ...
  },
  {
    ...
    examples: "remembering preferred content paths or user-selected settings.",
  },
  {
    id: 5,
...
export default function CookieConsent() {
  ...
  const handleRejectNonEssential = () => {
    setCategories((prev) =>
      prev.map((cat) => ({
        ...cat,
        enabled: cat.alwaysOn ? true : false,
      })),
    );
    setIsSaved(false);
  };
  ...
  const handleSaveChoices = () => {
    setIsSaved(true);
    // You can attach local storage persistence or API calls here
    ...
    <section id="manage" className="bg-[#f6f9fc] px-6 md:px-54 py-12">
      <div className="max-w-6xl mx-auto">
        ...
        <p className="text-[#566476] max-w-155 mb-4">
          Rejecting non-essential cookies is exactly as easy as accepting them.
          Essential cookies keep the site working and stay on.
        </p>
        ...
        <div className="bg-white rounded-2xl pt-4 shadow-sm mb-4">
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-5 p-6 border-b border-[#E2E8F0]">
            ...
            <div className="flex flex-wrap items-center gap-3">
              <button
                ...
                className="h-11 rounded-xl border cursor-pointer border-[#CDD7E3] bg-white px-6 font-semibold text-[#0F172A] transition hover:bg-slate-50 active:scale-95"
              >
              ...
              <button
                ...
                className="h-11 rounded-xl bg-[#13A594] border cursor-pointer border-[#13A594] px-7 font-semibold text-white transition hover:bg-teal-600 active:scale-95"
              >
                {isSaved ? "Saved!" : "Save Choices"}
              </button>
            </div>
          </div>
          ...
          {categories.map((item, index) => (
            <div
              key={item.id}
              className={`flex items-start justify-between gap-5 p-6 ${
            ...
            >
              <div className="flex-1">
                ...
                <p className="text-[12.5px] leading-6 text-[#7C8A9B]">
                  <span className="font-semibold text-[#7C8A9B]">
                    Examples:
                  </span>{" "}
                  {item.examples}
                </p>
              ...
              <div className="flex flex-col items-center gap-1">
                <button
                  ...
                  className={`relative h-8 w-13 rounded-full transition ${
                    item.alwaysOn
                      ? "bg-[#13A594] opacity-80 cursor-not-allowed"
                      : item.enabled
                        ? "bg-[#13A594] cursor-pointer"
                        : "bg-[#CDD7E3] cursor-pointer"
                  }`}
                >
                  <span
        ...
        <p className="text-[#7C8A9B] text-[13.1px] mb-4">
          Basic public medicine availability search is not blocked by
          non-essential cookie choices. Non-essential analytics,
          personalization, and third-party scripts do not run before consent
