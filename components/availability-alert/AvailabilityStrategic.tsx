      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="text-center">
          <h2 className="text-3xl font-semibold leading-10 tracking-[-.32px] text-[#000615] md:text-[32px]">
            Strategic{" "}
            <span className="text-[#0FAA87]">Awareness Scenarios</span>
          </h2>

          <p className="text-[#44474D]">
            Understanding where automated signal monitoring adds the most value.
          </p>
        </div>
        ...
        <div className="mt-12 grid gap-8 md:grid-cols-2">
          {scenarios.map(({ title, description, link, src, height, width }) => (
            <div
              key={title}
            ...
            >
              ...
              <h3 className="mt-5 font-semibold text-[#000615]">{title}</h3>

              <p className="mt-4 text-[#44474D]">{description}</p>
            </div>
          ))}
        </div>
        ...
        <div className="mt-10 flex flex-col gap-6 rounded-[16px] border-2 border-[#009A6533] bg-[#009A651A] p-5 sm:p-6 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex items-start gap-4">
            ...
            <p className="max-w-[680px] text-sm leading-6 font-bold text-[#000615] sm:text-base">
              <span className="font-extrabold">Scenario Safeguard:</span> Alerts
              support availability awareness. They do not replace medical
              advice, pharmacy consultation, or official prescription
              fulfillment protocols.
            </p>
          </div>

          <button
            onClick={() => router.push("/controlled-medicine-policy")}
            className="w-full rounded-full cursor-pointer bg-[#009A65] px-6 py-3 text-sm font-medium text-white transition hover:bg-[#037f58] sm:w-auto sm:px-8 sm:text-base"
          >
            Full Policy Details
          </button>
        </div>
