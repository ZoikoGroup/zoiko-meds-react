...
const PATHS = [
  {
    ...
    description:
      "Care coordinators, discharge planners, patient navigators, and clinic teams.",
  ...
  {
    icon: "building",
    title: "Provider organization access",
    description:
...
const ACCESS_INTERESTS = [
  "Care-team availability workflows",
  "Discharge planning support",
  "Patient navigation tools",
  "Organization-wide governed access",
  "Not sure yet",
] as const;
...
export default function CareTeamAccessPathwaySection() {
            ...
            <>
              <Reveal index={0}>
                <h2 className="font-[var(--font-plus-jakarta-sans)] text-3xl font-bold leading-tight text-[#0F1F4E] sm:text-[2.25rem]">
                  ...
                  <span style={{ color: ACCENT }}>team pathway.</span>
                </h2>
              </Reveal>
...
function Reveal({
  ...
  return (
    <div
      className="animate-[careTeamPathwayFadeUp_0.6s_ease-out_forwards]"
      style={{ opacity: 0, animationDelay: `${index * 100}ms` }}
    >
      {children}
      <style jsx>{`
        @keyframes careTeamPathwayFadeUp {
          from {
            opacity: 0;
...
function PathCard({
  ...
  return (
    <div
      className="group flex flex-1 flex-col rounded-2xl border border-[#E7EAF1] bg-white p-6 transition-all duration-300 ease-out animate-[careTeamPathwayFadeUp_0.6s_ease-out_forwards] hover:-translate-y-1"
      style={{
        opacity: 0,
...
function PathIcon({ name }: { name: "users" | "building" | "search" }) {
...
function BriefingForm({
  ...
  return (
    <div
      className="flex w-full flex-col rounded-2xl border border-[#E7EAF1] bg-white p-8 shadow-[0_20px_45px_-30px_rgba(15,31,78,0.25)] animate-[careTeamPathwayFadeUp_0.6s_ease-out_forwards]"
      style={{ opacity: 0, animationDelay: "550ms" }}
    >
      <h3 className="text-[18px] font-bold text-[#0F1F4E]">
        Request a care team access briefing
      </h3>
      <p className="mt-1.5 text-[13.5px] leading-relaxed text-[#5B6478]">
        Tell us about your organization and care-team access model. No
      ...
      <form className="mt-6 flex flex-1 flex-col space-y-5" onSubmit={onSubmit} noValidate>
        <Field id="care-team-email" label="Work email" error={errors.email}>
          <input
            id="care-team-email"
            name="email"
            type="email"
        ...
        <Field id="care-team-name" label="Full name" error={errors.fullName}>
          <input
            id="care-team-name"
            name="fullName"
            type="text"
        ...
        <Field
          id="care-team-access-interest"
          label="Primary access interest"
          optional
        >
          <SelectField
            id="care-team-access-interest"
            name="accessInterest"
            value={form.accessInterest}
            onChange={(value) => onChange("accessInterest", value)}
            placeholder="Select access interest"
            options={ACCESS_INTERESTS}
            hasError={false}
          />
        </Field>

        <Field id="care-team-note" label="Brief note" optional>
          <textarea
            id="care-team-note"
            name="note"
            value={form.note}
            onChange={(e) => onChange("note", e.target.value)}
            placeholder="Your care-team workflow, patient-support need, or access model"
            rows={3}
        ...
        >
          ...
          {status === "submitting"
            ? "Submitting..."
            : "Request Care Team Access Briefing"}
        </button>
