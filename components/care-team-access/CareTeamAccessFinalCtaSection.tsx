...
export default function CareTeamAccessFinalCtaSection() {
            ...
            <>
              <Reveal index={0}>
                <h2 className="font-[var(--font-plus-jakarta-sans)] text-2xl font-bold leading-tight text-white sm:text-[1.85rem]">
                  Equip care teams with safer availability guidance.
                </h2>
              </Reveal>

              <Reveal index={1}>
                <p className="mx-auto mt-4 max-w-xl text-[14px] leading-relaxed text-[#A9B2C8]">
                  Use ZoikoMeds to support medicine availability
                  conversations, patient self-service, saved searches,
                  alerts, and pharmacy confirmation — without replacing
                  clinical care.
                </p>
              </Reveal>
...
function Reveal({
  ...
  return (
    <div
      className="animate-[careTeamFinalCtaFadeUp_0.6s_ease-out_forwards]"
      style={{ opacity: 0, animationDelay: `${index * 100}ms` }}
    >
      {children}
      <style jsx>{`
        @keyframes careTeamFinalCtaFadeUp {
          from {
            opacity: 0;
