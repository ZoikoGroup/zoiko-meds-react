...
const ACCENT = "#0FAA87";
const HERO_IMAGE_SRC = "/images/medical-disclaimer-hero.webp";

export default function MedicalDisclaimerHeroSection() {
      ...
      <div className="mx-auto max-w-6xl px-6 lg:px-8">
        <div className="grid grid-cols-1 items-center gap-10 lg:grid-cols-2 lg:gap-14">
          ...
          <div className="flex flex-col">
            ...
            {/* Headline — two lines of text, partial teal accent */}
            <Reveal index={2} active={mounted}>
              <h1 className="text-[2.3rem] font-extrabold leading-[1.1] tracking-tight sm:text-[2.7rem]">
                <span className="text-[#0F1F4E]">ZoikoMeds provides medicine availability information, </span>
            ...
            <Reveal index={3} active={mounted}>
              <p className="mt-5 text-[14.5px] leading-relaxed text-[#5B6478]">
                ZoikoMeds helps users search confidence-based medicine
                availability signals from participating verified pharmacies.
                It does not diagnose, treat, prescribe, dispense, sell,
            ...
            <Reveal index={6} active={mounted}>
              <p className="mt-5 flex items-start gap-2 text-[12.5px] leading-relaxed text-[#5B6478]">
                <InfoIcon />
                <span>
                  Always confirm availability, prescription requirements,
                  eligibility, and pharmacy instructions directly with the
                  pharmacy or a qualified healthcare professional before
                  acting.
                </span>
              </p>
            </Reveal>
          ...
          <Reveal index={3} active={mounted}>
            <div className="relative overflow-hidden rounded-2xl shadow-[0_16px_48px_-16px_rgba(15,31,78,0.18)]">
              <Image
                src={HERO_IMAGE_SRC}
                alt="Healthcare professional holding a ZoikoMeds medicine bottle in a pharmacy setting"
                width={680}
                height={520}
...
function Reveal({ children, index, active }: { children: React.ReactNode; index: number; active: boolean }) {
  return (
    <div style={{ opacity: active ? undefined : 0, animation: active ? `medDiscHeroFadeUp 0.6s ease-out ${index * 80}ms both` : "none" }}>
      {children}
      <style>{`
        @keyframes medDiscHeroFadeUp {
