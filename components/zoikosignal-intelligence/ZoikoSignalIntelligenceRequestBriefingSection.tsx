...
const ORG_TYPES = [
  ...
  "Manufacturer or life sciences company",
...
export default function ZoikoSignalIntelligenceRequestBriefingSection() {
  ...
  return (
    <section id="request" ref={ref} className="relative w-full bg-[#F4F6FA] py-20 sm:py-24">
      <div className="mx-auto max-w-6xl px-6 lg:px-8">
        {/* ---------------- Header (left-aligned within centered block) ---------------- */}
        <div>
          {mounted ? (
            <>
              <Reveal index={0}>
              ...
              <Reveal index={2}>
                <p className="mt-3 max-w-2xl text-[14.5px] leading-relaxed text-[#5B6478]">
                  ...
                  request to the right intelligence, public-sector,
                  commercial, security, or governance team.
                </p>
              </Reveal>
            </>
          ) : (
        ...
        {/* ---------------- Closing disclaimer ---------------- */}
        <div className="mt-6">
          {mounted ? <DisclaimerNote /> : <DisclaimerNoteSkeleton />}
        </div>
      </div>
    </section>
  );
...
function Reveal({
  ...
  return (
    <div
      className="animate-[zoikoSignalBriefingFadeUp_0.6s_ease-out_forwards]"
      style={{ opacity: 0, animationDelay: `${index * 90}ms` }}
    >
      {children}
      <style jsx>{`
        @keyframes zoikoSignalBriefingFadeUp {
          from {
            opacity: 0;
...
function BriefingForm({
  ...
  return (
    <div
      className="rounded-2xl border border-[#E7EAF1] bg-white p-8 shadow-[0_20px_45px_-30px_rgba(15,31,78,0.25)] animate-[zoikoSignalBriefingFadeUp_0.6s_ease-out_forwards]"
    ...
    >
      <form className="space-y-6" onSubmit={onSubmit} noValidate>
        ...
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
          <Field id="zoikosignal-interest" label="Primary interest" optional>
            <SelectField
              id="zoikosignal-interest"
        ...
        {/* Brief note */}
        <Field id="zoikosignal-note" label="Brief note" optional>
          <textarea
            id="zoikosignal-note"
            value={form.note}
            onChange={(e) => onChange("note", e.target.value)}
            placeholder="Your intelligence interest, jurisdiction, or evaluation goal."
            rows={3}
        ...
        <p className="flex items-start gap-1.5 text-[11.5px] leading-relaxed text-[#A6ADBD]">
          ...
          Don&apos;t include PHI, patient data, prescription records,
          exact pharmacy stock, or confidential data in this form.
        </p>
        ...
        {status === "success" && (
          <p className="flex items-center gap-2 text-[13px] font-medium text-[#0E8F70]">
            ...
            Your ZoikoSignal™ briefing request has been submitted.
...
function DisclaimerNote() {
  return (
    <div
      className="rounded-2xl border border-[#E7EAF1] border-l-4 border-l-[#0F1F4E] bg-[#EDEFF4] p-5 animate-[zoikoSignalBriefingFadeUp_0.6s_ease-out_forwards]"
      style={{ opacity: 0, animationDelay: "500ms" }}
    >
      <p className="text-[12.5px] leading-relaxed text-[#5B6478]">
        ZoikoSignal™ provides governed medicine availability intelligence
        based on aggregated and privacy-safe signals from participating
        verified pharmacies and approved sources. ZoikoSignal™ does not
        provide medical advice, prescribe, dispense, sell, deliver,
        reserve, recommend, allocate, guarantee medicines, validate
        prescriptions, expose exact public pharmacy stock, sell
        identifiable patient-level data, or replace official shortage
        authorities. Enterprise access and outputs are subject to
        contract, privacy controls, jurisdiction-specific laws,
        data-use restrictions, thresholds, and approved governance
        rules.
      </p>
    </div>
  );
...
function DisclaimerNoteSkeleton() {
  return (
