...
export default function ConfirmationRequestsFinalCtaSection() {
            ...
            <>
              <Reveal index={0}>
                <h2 className="font-[var(--font-plus-jakarta-sans)] text-2xl font-bold leading-tight text-white sm:text-[1.85rem]">
                  Respond to availability inquiries with more control.
                </h2>
              </Reveal>

              <Reveal index={1}>
                <p className="mx-auto mt-4 max-w-xl text-[14px] leading-relaxed text-[#A9B2C8]">
                  Set up confirmation requests through ZoikoMeds to reduce
                  repetitive calls, protect exact stock, and guide
                  patients toward safer next steps.
                </p>
              </Reveal>
...
function Reveal({
  ...
  return (
    <div
      className="animate-[confirmationFinalCtaFadeUp_0.6s_ease-out_forwards]"
      style={{ opacity: 0, animationDelay: `${index * 100}ms` }}
    >
      {children}
      <style jsx>{`
        @keyframes confirmationFinalCtaFadeUp {
          from {
            opacity: 0;
  ...
  return (
    <div className="flex flex-col items-center gap-4">
      ...
      <div className="mt-3 flex gap-3">
        <div className="h-11 w-56 animate-pulse rounded-xl bg-white/10" />
